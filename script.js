...
let checkDogs = function (dogsJulia, dogsKate) {
  let arrDogs = dogsJulia.concat(dogsKate);
  arrDogs.forEach(function (age, i) {
    let type = age < 3 ? 'a puppy 🐶' : 'an adult';
...
//   const inHumanAge = ages.map(age => {
//     if (age <= 2) return age * 2;
//     else return 16 + age * 4;
//   });
//   console.log(inHumanAge);
//   const less18 = inHumanAge.filter(age => age > 18);
//   console.log(less18);
//   const averageAge = less18.reduce((acc, age) => acc + age, 0) / less18.length;
//   // return acc / less18.length;
...
dogSarah.curFood > dogSarah.recommendedFood ? console.log('eating too much') : console.log(`eating too little`);

// challenge 3
let ownersEatTooMuch = new Array();
let ownersEatTooLittle = new Array();
dogs.map(dog => dog.curFood > dog.recommendedFood ? ownersEatTooMuch.push(dog.owners) : ownersEatTooLittle.push(dog.owners));
...
// 5
let exactAmount = dogs.some(dog => dog.curFood === dog.recommendedFood);
console.log(exactAmount);

//6
let okayAmount = dogs.some(dog => dog.curFood > (dog.recommendedFood * 0.90) && dog.curFood < (dog.recommendedFood * 1.10));
console.log(okayAmount);

// 7
let okayAmountDog = dogs.filter(dog => {
  if (dog.curFood > (dog.recommendedFood * 0.90) && dog.curFood < (dog.recommendedFood * 1.10)){
    return dog;
  }});
console.log(okayAmountDog);
